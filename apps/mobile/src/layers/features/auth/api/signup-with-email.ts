...
type Error = Err<
	"EMAIL_ALREADY_REGISTERED" | "CAPTCHA_VERIFICATION_FAILED" | "TOO_MANY_REQUESTS" | "SERVER_ERROR" | "UNKNOWN_ERROR",
	{
		errorMessage: string;
	}
>;
...
export const signup = async (
	...
	const res = await edenFetch.auth.signup.post(
		{
	...
	if (status === 400) {
		if (value.code === "EMAIL_ALREADY_REGISTERED") {
			return err("EMAIL_ALREADY_REGISTERED", { errorMessage: "メールアドレスはすでに登録されています" });
		}
