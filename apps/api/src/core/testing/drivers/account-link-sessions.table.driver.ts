...
export class AccountLinkSessionsTableDriver {
	constructor(private readonly db: D1Database) {}

	public async save(raw: RawAccountLinkSession): Promise<void> {
		await this.db
			.prepare(
				"INSERT INTO account_link_sessions (id, user_id, code, secret_hash, email, provider, provider_user_id, expires_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
			)
			.bind(
				raw.id,
				raw.user_id,
