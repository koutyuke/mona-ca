...
export const verificationEmailTemplate = (verification: EmailVerification): EmailContext => {
	const from = MC_DEFAULT_EMAIL_ADDRESS;
	const to = verification.email;
	const subject = `メールアドレス検証コードは ${verification.code} です。`;
	const text = `メールアドレスが正しく登録されていることを確認するため、以下のコードを入力してください。

コード：${verification.code}
