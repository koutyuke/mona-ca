...
type TokenToId<T extends SessionTokens> = Extract<SessionTokenAndIdMap, [T, unknown]>[1];
type IdToToken<T extends SessionIds> = Extract<SessionTokenAndIdMap, [unknown, T]>[0];

export const parseSessionToken = <T extends SessionTokens>(token: T): { id: TokenToId<T>; secret: string } | null => {
...
export const formatSessionToken = <T extends SessionIds>(id: T, secret: string): IdToToken<T> => {
