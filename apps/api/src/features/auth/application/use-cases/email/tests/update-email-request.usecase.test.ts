...
import { UpdateEmailRequestUseCase } from "../update-email-request.usecase";
...
describe("UpdateEmailRequestUseCase", () => {
	...
	it("should create email verification request successfully for new email", async () => {
		...
		const result = await updateEmailRequestUseCase.execute("new@example.com", userIdentity);
	...
	it("should return EMAIL_ALREADY_REGISTERED error when trying to use current email", async () => {
		authUserMap.set(userRegistration.id, userRegistration);

		const result = await updateEmailRequestUseCase.execute("test@example.com", userIdentity);
	...
	it("should return EMAIL_ALREADY_REGISTERED error when email is already used by another user", async () => {
		...
		const result = await updateEmailRequestUseCase.execute("existing@example.com", userIdentity);
	...
	it("should generate 8-digit numeric verification code", async () => {
		...
		const result = await updateEmailRequestUseCase.execute("new@example.com", userRegistration);
