...
describe("EmailVerificationRequestRepository.deleteByUserId", () => {
	beforeEach(async () => {
	...
	test("should delete data in database", async () => {
		...
		await emailVerificationRequestRepository.deleteByUserId(emailVerificationRequest.userId);

		const results = await emailVerificationRequestTableDriver.findByUserId(userRegistration.id);
